...
export default function EventForm({ mode, eventId, parentId, onClose, onResult }: Props) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Trigger location validation when location field changes
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const body = mode === 'create' && parentId
        ? { ...formData, parentId, projectId: selectedProject?.id }
          ...
          <TextField
            ...
            helperText={
              formData.latitude && formData.longitude 
                ? `📍 Validated: ${formData.city || formData.region || formData.country || 'Unknown location'}`
                : formData.location.length > 2 
                  ? 'Searching for location matches...'
                  : 'Enter location name to validate'
            }
            InputProps={{
